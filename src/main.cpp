...
void loop() {
    ...
    if (currentlyPuffing && !isPuffing) { isPuffing = true; puffStartTime = currentTime; actionFired = false; }
    else if (!currentlyPuffing && isPuffing) {
        isPuffing = false;
        if ((currentTime - puffStartTime) < LONG_PUFF_MS) { puffCount++; lastPuffTime = currentTime; }
    } else if (currentlyPuffing && isPuffing && (currentTime - puffStartTime) >= LONG_PUFF_MS && !actionFired) {
        executeHomeSequence(); actionFired = true; puffCount = 0;
    }
